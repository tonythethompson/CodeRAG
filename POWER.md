...
Before using this power, set the `CODERAG_API_URL` environment variable:

- **`CODERAG_API_URL`**: Your deployed CodeRAG API Gateway URL.
  - **How to get it:**
    1. Deploy the stack: `sam build && sam deploy --guided`
    2. Note the `ApiUrl` value from stack outputs
    3. Looks like: `https://abc123def.execute-api.us-east-1.amazonaws.com/dev`
...
- Check `CODERAG_API_URL` env var is set
